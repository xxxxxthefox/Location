...
if ($method === 'POST' && $action === 'change_skin') {
    $data = json_decode(file_get_contents('php://input'), true);
    $playerName = $data['player'] ?? '';
    $skinUrl = $data['skin_url'] ?? '';
    
    $ch = curl_init("https://api.mineskin.org/generate/url");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['url' => $skinUrl]));
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
    $response = curl_exec($ch);
    curl_close($ch);
    
    jsonResponse(['success' => true, 'message' => 'Skin change requested', 'data' => json_decode($response, true)]);
}
